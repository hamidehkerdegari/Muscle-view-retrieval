...
# This script is used to generate ground truth annotations for all of standard views in the video one (T1) that is being compared with the  whole sequence of video two (T2).
...
class MuscleUltrasoundDataset:
    ...
    def process(self, index: int):
        ...
        features_folder_path = self.video_file_paths[index].replace(".mp4", "_gt")
        if not os.path.exists(features_folder_path):
            os.system("mkdir " + features_folder_path)

        if len([fs for fs in os.listdir(features_folder_path) if fs.endswith(".npy")]) == 0:
            if not os.path.exists(features_folder_path):
                os.mkdir(features_folder_path)
            cap = cv.VideoCapture(video_file_path)
            if cap.isOpened() == False:
                print('[ERROR] [MuscleUltrasoundDataset.__getitem__()] Unable to read video ' + self.video_filenames[
                    index])
                exit(-1)
            ...
            for sec_id, annotation in enumerate(json_data['annotations']):
                print(annotation["number_of_frames"])
                section_frames = self._extract_frames(cap, annotation)

                np.save(features_folder_path + "/" + str(sec_id) + ".npy", np.array(section_frames))



muscle = MuscleUltrasoundDataset(dataset_path='dataset path') # path to dataset
