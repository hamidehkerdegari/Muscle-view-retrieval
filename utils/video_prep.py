...
# This script is used to generate numpy data for model training.
    ...
    def process(self, index: int):
        ...
        for s, annotation in enumerate(json_data['annotations']):
            ...
            if section_frames is not None:
                section_frames = np.array(section_frames)
                print(section_frames.shape)
                for f in range(0, section_frames.shape[0]-1):
                    if comp_frames(section_frames[f:f+2, :, :, :]):
                        data.append(section_frames[f])
                    else:
                        print(f)
        return data
...
for i in range(len(muscle.video_file_paths)):
    ...
    data = muscle.process(i)
    np.save(muscle.video_file_paths[i].split('.')[0]+'_train.npy', data)
